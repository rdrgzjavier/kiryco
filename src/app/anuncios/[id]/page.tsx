      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
        <article>
          ...
          <div className="mb-4 flex flex-wrap gap-2">
            <span className="chip">{category?.name}</span><VerifiedBadge verified={listing.verified} /><StatusBadge status={listing.status} />
          </div>
          ...
          <div className="mt-5 flex flex-wrap gap-2">{listing.tags.map((tag) => <span key={tag} className="rounded-full bg-soft px-3 py-1 text-xs font-semibold text-slatecopy">{tag}</span>)}</div>
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {[["Zona", `${listing.municipality} · ${listing.area}`], ["Centro relacionado", center?.name ?? "No indicado"], ["Edad recomendada", ageLabel(listing)], ["Precio", listing.priceLabel ?? (listing.price ? `${listing.price} €` : "Consultar")], ["Estado", listing.condition ?? "No aplica"], ["Disponibilidad", listing.availability ?? "Consultar"]].map(([label, value]) => (
              <div key={label} className="rounded-xl border border-line bg-panel p-4"><p className="label">{label}</p><p className="mt-2 font-semibold text-ink">{value}</p></div>
            ))}
          </div>
          <section className="mt-8 card p-6">
            <h2 className="text-2xl font-semibold text-ink">Información específica</h2>
            <dl className="mt-4 grid gap-3">
              {Object.entries(listing.details).map(([key, value]) => <div key={key} className="flex justify-between gap-4 border-b border-line pb-3 text-sm"><dt className="font-semibold capitalize text-slatecopy">{key.replace(/([A-Z])/g, " $1")}</dt><dd className="text-right text-ink">{value}</dd></div>)}
            </dl>
          </section>
        </article>
        <aside className="space-y-4">
          ...
          <div className="rounded-2xl border border-coral/30 bg-coral/5 p-5"><ShieldAlert className="text-coral" size={24} aria-hidden /><h3 className="mt-3 font-semibold text-ink">Aviso de seguridad</h3><p className="mt-2 text-sm leading-6 text-slatecopy">Plataforma dirigida exclusivamente a adultos. No publiques fotos de menores, nombres completos, clase, horarios personales ni datos sensibles.</p></div>
        </aside>
      </div>
    </div>
  );

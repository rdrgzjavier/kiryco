  ...
  return (
    <div className="section-shell">
      <div className="grid gap-8 lg:grid-cols-[1fr_340px]">
        <article>
          <ImageWithFallback src={initiative.image} fallbackSrc="/images/cards/actividad.svg" alt={`Imagen de ${initiative.name}`} className="h-64 w-full rounded-2xl border border-line object-cover sm:h-80" />
        ...
        <aside className="card h-fit p-6">
          ...
          {officialUrl ? (
            <a href={officialUrl} target="_blank" rel="noreferrer" className="btn-primary mt-5 w-full">Web oficial<ExternalLink size={16} /></a>
          ) : (
            <p className="mt-5 rounded-xl border border-line bg-soft p-4 text-sm font-semibold leading-6 text-slatecopy">Fuente oficial pendiente de validar por Tenlo.</p>
          )}
        </aside>
      </div>
    </div>
